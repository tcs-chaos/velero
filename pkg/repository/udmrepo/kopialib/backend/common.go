...
func SetupConnectOptions(ctx context.Context, repoOptions udmrepo.RepoOptions) repo.ConnectOptions {
	...
	return repo.ConnectOptions{
		CachingOptions: content.CachingOptions{
			// softLimit 80%
			ContentCacheSizeBytes:  int64((dataCacheLimit / 5 * 4) << 10),
			MetadataCacheSizeBytes: int64((metadataCacheLimit / 5 * 4) << 10),
			// hardLimit 100%
			ContentCacheSizeLimitBytes:  int64(dataCacheLimit << 10),
			MetadataCacheSizeLimitBytes: int64(metadataCacheLimit << 10),
